...
async def handle_symbol(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply_keyboard = [["Scalping", "Day Trading"], ["Swing Trading", "Long-Term"], ["Multi Timeframe"]]
    await update.message.reply_text(f"📈 Pilih strategi trading untuk {symbol}:", reply_markup=ReplyKeyboardMarkup(reply_keyboard, resize_keyboard=True, one_time_keyboard=True))
    return SELECTING_TIMEFRAME

@only_allowed
async def handle_timeframe_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    timeframe_map = {
        "Scalping": "15m", "Day Trading": "1h", "Swing Trading": "1d",
        "Long-Term": "1w", "Multi Timeframe": "4h"
    }
    ...
    try:
        ...
        ohlc = [{"time": k[0], "open": float(k[1]), "high": float(k[2]), "low": float(k[3]), "close": float(k[4]), "volume": float(k[5])} for k in klines]
